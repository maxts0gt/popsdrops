-- Automate data-rights account deletion.
--
-- Product decision: privacy deletion is a real lifecycle workflow. It is the
-- one approved scheduled processor in this area, not dormant platform sync or
-- old architecture drift.
...
alter table public.data_rights_requests
  add constraint data_rights_requests_status_check check (
    status in (
      'pending',
      'scheduled',
      'reviewing',
...
create index if not exists data_rights_requests_due_deletions_idx
  on public.data_rights_requests (scheduled_for, created_at)
...
revoke all on function app_private.process_due_data_deletion_requests(integer)
  from public;
...
begin
  if exists (
    select 1
    from cron.job
    where jobname = 'process-due-data-deletions-hourly'
  ) then
    perform cron.unschedule('process-due-data-deletions-hourly');
  end if;

  perform cron.schedule(
    'process-due-data-deletions-hourly',
    '17 * * * *',
    $cron$select app_private.process_due_data_deletion_requests(100);$cron$
  );
end
$data_rights_cron$;

comment on function app_private.process_due_data_deletion_requests(integer) is
  'Processes due privacy deletion requests in small batches, anonymizing user-facing PII while preserving legally required campaign, payment, agreement, consent, and audit records.';
