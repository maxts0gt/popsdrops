-- Harden the automatic deletion processor so active Supabase Auth identity
-- fields are tombstoned alongside application profile data.
...
comment on function app_private.process_due_data_deletion_requests(integer) is
  'Processes due privacy deletion requests in small batches, tombstoning Supabase Auth identity fields and anonymizing user-facing PII while preserving legally required campaign, payment, agreement, consent, and audit records.';
