...
function getReportDisplayTitle(data: ReportExportData): string {
...
function buildHtmlMetricTile(
  metric: { key?: string; label: string; value: string; detail?: string },
  index: number,
): string {
  const metricKey = metric.key?.trim() || `metric-${index}`;
...
function getReportEvidenceTrail(data: ReportExportData): string {
  ...
  const trail = source ? `${evidenceLine} / ${source.value}` : evidenceLine;
...
function getReportTrustDecision(data: ReportExportData): string {
          ...
          return `<article class="comparison-row">
            ...
            <div class="comparison-body">
              ...
              <div class="comparison-line comparison-line--meta">
                ...
                <span>Views ${escapeHtml(creator.views)}</span>
                <span>Engagements ${escapeHtml(creator.engagements)}</span>
                <span>CPE ${escapeHtml(creator.cpe)}</span>
              </div>
            </div>
          </article>`;
        })
        ...
        ${data.trust
          .map(
            (item) => {
              ...
              const dateClass = value.includes("/") ? " trust-card--date" : "";
