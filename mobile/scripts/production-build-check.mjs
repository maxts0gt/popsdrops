...
function validateProductionBuildProfile(easConfig) {
  const issues = [];
...
function validateAppReleaseMetadata(appConfig) {
  const issues = [];
  ...
  if (!expo?.scheme) {
    issues.push("expo.scheme is required for production auth redirects.");
  }

  if (!ios?.bundleIdentifier) {
    issues.push("expo.ios.bundleIdentifier is required for production builds.");
  }

  if (!android?.package) {
    issues.push("expo.android.package is required for production builds.");
  }
  ...
  if (ios?.config?.usesNonExemptEncryption !== false) {
    issues.push("iOS usesNonExemptEncryption must be false.");
  }
...
function validateRequiredReleaseFiles(mobileRoot) {
  const issues = [];
  const requiredFiles = [
    "store-metadata.md",
  ...
  for (const filename of requiredFiles) {
    if (!existsSync(path.join(mobileRoot, filename))) {
      issues.push(`${filename} is required before production builds.`);
    }
  }
...
const issues = [
  ...missingEnv.map((key) => `${key} is required for production builds.`),
  ...validateProductionBuildProfile(easConfig),
  ...validateEasProjectIdentity(appConfig),
  ...validateAppReleaseMetadata(appConfig),
  ...validateRequiredReleaseFiles(mobileRoot),
];
...
console.log("Mobile production build check passed.");
