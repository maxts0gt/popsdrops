...
## Product Doctrine - Global Proof Room

PopsDrops is not trying to win by being another influencer marketplace, creator database, chat inbox, or generic AI matching tool. The enterprise wedge is the **Global Proof Room**: a private cross-border campaign workspace where a brand can brief, localize, govern, collect evidence, and produce executive-ready reports across markets without losing brand control.

The simulated top-brand buyer pain is: "I can find creators, but I cannot confidently run a local-market creator campaign that is on-brand, compliant, understandable to creators, and provable to leadership." Every major roadmap decision should be tested against that sentence.

Prioritize features that strengthen the Global Proof Room:
- Structured campaign brief and localized creator instructions
- Rules, NDA, usage-rights, disclosure, and claim-safety gates before locked materials
- Content approval, correction loops, publishing windows, and missed-work visibility
- Evidence-first reporting with AI extraction as assistance only, creator confirmation as truth, and brand review as trust
- Market, platform, creator, and evidence-source reporting that can survive executive, legal, and finance review

Pause features that pull PopsDrops toward an undifferentiated marketplace: public creator supply races, generic chat, automatic platform-token metric sync, broad agency operations, or AI matching that is not tied to private campaign execution and proof. Discovery supports private campaigns; proof, governance, localization, and executive confidence are the core wedge.

## Current Quality Goal - Luxury Web First

When product direction is in tension, prioritize making the web/private-beta Global Proof Room feel worthy of Chanel or Hermès before chasing mobile store-release evidence. Mobile creator validation matters, but it must not distract from unfinished core product quality: campaign governance, creator evidence submission, brand review, admin visibility, and executive-ready self-serve reports.

Evidence for "good enough" must be concrete: focused tests, browser smoke screenshots, export artifacts, and a clear explanation of why the workflow supports an invite-led, high-trust campaign OS. Do not claim luxury-grade readiness from intent, mocks, or unfinished scaffolding.
...
- **CI/CD:** GitHub Actions is reserved for post-merge production deploys only. Before merging, run focused local checks plus the relevant smoke command; use `npm run smoke:release` as the full web/private-beta release gate when the change touches the product loop. Mobile release checks stay separate under `npm run smoke:mobile-release` until the web Global Proof Room is strong enough to return to app-store validation.
...
Always close work updates with the next move. State what is next, why it matters, and how it fits the PopsDrops journey toward a self-running, invite-led campaign OS. This keeps sessions story-driven instead of drifting into disconnected small tasks. If the next move is uncertain, give the honest decision point and the recommended direction.
...
For local browser smoke tests in Codex, use the Codex in-app browser first through the Browser Use plugin with the `iab` backend. It is the fastest, most direct way to inspect localhost UI, DOM state, screenshots, and console health. Fall back to Playwright CLI only when the in-app browser runtime is unavailable or blocked. If localhost refuses the connection, start the dev server and retry in the in-app browser before choosing a fallback.

Never run `next build` at the same time as a dev server or browser smoke test. Build and dev both touch generated `.next` state, and overlapping them can create false route failures such as valid routes returning 404. Stop the dev server before production builds; if this happens, clear `.next`, restart cleanly, and rerun the failing smoke.
...
- `create-stripe-checkout-session` - Stripe Checkout Session creation for campaign service fees; Stripe secret keys live only in Supabase secrets
